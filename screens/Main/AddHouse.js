...
*/
  ...
  const getHouseInfo = async () => {
    ...
    const foundItem = addHouseList?.find(item => item.index === props?.route.params.index);
      ...
      if (response.data.errYn === 'Y') {
        ...
        if (result.data?.property?.acquisitionDate) {
          if (result.data?.property?.acquisitionDate !== null) {
            ...
            setaddHouseInfo([{ acquisitionDate: new Date(year, month, day) }]);
            setDirectAcquisitionDate(true);
          } else {
            setaddHouseInfo([{ acquisitionDate: null }]);
  ...
  useEffect(() => {
    var beforebuyprice = addHouseList.find(el => el.index === props?.route.params.index)?.buyPrice;
                  ...
                  if (canProceed) {
                    ...
                    if (!addHouseInfo.acquisitionDate && foundItem.buyPrice === null) {
                      dispatch(editAddHouseList({
                        ...
                        buyDate: addHouseInfo.acquisitionDate ? addHouseInfo.acquisitionDate : foundItem.buyDate,
                        buyPrice: foundItem.buyPrice,
                        complete: false,
                    ...
                    console.log('addHouseList2', addHouseList);
                  }
                }} style={{
                  width: '100%',
                  height: 50, // height 값을 숫자로 변경하고 단위 제거
              ...
              <Button active={selectedDate}
                ...
                onPress={async () => {
                  ...
                  if (canProceed) {
                    ...
                    await getHouseInfo();
                    //console.log('selectedDate', selectedDate);
                    console.log('addHouseInfo', addHouseInfo);
      ...
      <Container style={{ width: width }}>
        ...
        <><IntroSection2 style={{ width: width }}>
          ...
          <View
            ...
            {addHouseInfo?.acquisitionDate && <Calendar
              ReservationYn='N'
              currentPageIndex={1}
              //      minDate={new Date(new Date('2024-05-02').setHours(0, 0, 0, 0))}
              setSelectedDate={setSelectedDate}
              selectedDate={addHouseInfo.acquisitionDate ? addHouseInfo.acquisitionDate.setHours(0, 0, 0, 0) : new Date().setHours(0, 0, 0, 0)}
              currentDate={addHouseInfo.acquisitionDate ? addHouseInfo.acquisitionDate.setHours(0, 0, 0, 0) : new Date().setHours(0, 0, 0, 0)}
            ...
            <DropShadow style={styles.dropshadow}>
              <Button active={selectedDate}
                ...
                onPress={async () => {
                  ...
                  const canProceed = await handleNetInfoChange(state);
                  if (canProceed) {
                    foundItem = addHouseList?.find((el) => el.index === props?.route.params.index);
